...
namespace Tweetbook.Services.Indentity
{
    public class IdentityService : IIdentityService
    {
        ...
        public IdentityService(UserManager<IdentityUser> userManager, JwtOptions jwtOptions, TokenValidationParameters tokenValidationParameters, DataContext dataContext)
        {
            ...
            _validationTokenParameters = tokenValidationParameters;
        ...
        public async Task<AuthenticationResult> RegisterAsync(string email, string password)
        {
            ...
            if (existingUser != null)
            {
                return new AuthenticationResult
                {
                    Errors = new string[] { "User with this email address already exists." }
                };
            }
        ...
        private async Task<AuthenticationResult> GenerateAuthenticationResultForUserAsync(IdentityUser user)
        {
            ...
            await _dataContext.RefreshTokens.AddAsync(refreshToken).ConfigureAwait(false);
            await _dataContext.SaveChangesAsync().ConfigureAwait(false);
        ...
        private ClaimsPrincipal GetPrincipalFromToken(string token)
        {
            ...
            try
            {
                var validationTokenParameters = _validationTokenParameters.Clone();
                validationTokenParameters.ValidateLifetime = false;
                var principal = tokenHandler.ValidateToken(token, validationTokenParameters, out var validatedToken);
